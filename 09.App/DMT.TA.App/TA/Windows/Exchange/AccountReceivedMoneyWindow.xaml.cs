...
namespace DMT.TA.Windows.Exchange
{
    ...
    public partial class AccountReceivedMoneyWindow : Window
    {
        ...
        private void CheckUser()
        {
            ...
            {
                // write log.
                msg = string.Format("ACCOUNT RECEIVED MONEY - USER NOT FOUND. USERID: {0}", userId);
                med.Info(msg);
        ...
        private void CheckUser(User user)
        {
            ...
            {
                // write log.
                msg = "ACCOUNT RECEIVED MONEY - USER NOT FOUND.";
                med.Info(msg);
            }
            else
            {
                // write log.
                msg = string.Format("ACCOUNT RECEIVED MONEY - USER FOUND. USERID: {0}, USERNAME: {1}", user.UserId, user.FullNameTH);
                med.Info(msg);
            }

            if (null != user && !string.IsNullOrWhiteSpace(user.RoleId))
            {
                var roles = new List<string>(TAApp.Permissions.ACCOUNTS);
